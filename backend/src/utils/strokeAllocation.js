 ...
 */
export function allocateStrokesFor9Holes(strokes, holes) {
  if (strokes === 0) return {};

  // Filter out turbo holes
  const normalHoles = holes.filter(h => !h.turbo || h.turbo === 1);
 ...
 */
export function calculateStrokeAllocation(players, courseHoles) {
  ...
  for (const fromPlayer of players) {
    ...
    for (const toPlayer of players) {
      ...
      allocation[fromPlayer.id][toPlayer.id] = {
        front9: allocateStrokesFor9Holes(strokes, front9),
        back9: allocateStrokesFor9Holes(strokes, back9)
      };
    }
  }
